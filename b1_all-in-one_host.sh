...
ARQUIVO="$SCRIPT_DIR/variaveis.sh"
...
#verifica se a variavél all_in_one está definida como sim.
if [[ "$all_in_one" != "sim" ]]; then
  echo "Erro: all_in_one não está definido como 'sim'."
  exit 1
fi
...
if [ -b "/dev/$disk_block" ]; then
    ...
    echo "o disco /dev/$disk_block não foi encotrado, veja se a variável foi definida corretamente e se o disco existe"
    exit 1
fi
else
...
# instalando curl, openssh-server e git
echo "instalando 'curl', 'openssh-server' e 'git'..."
sudo apt install curl openssh-server git -y &>/dev/null

# configurar arquivo do netplan
...
#echo "chamando o script 'designate.sh'..."
#bash "$SCRIPT_DIR/designate.sh"

#echo "chamando o script 'heat.sh'..."
#bash "$SCRIPT_DIR/heat.sh"

echo "chamando o script 'all-in-one_update.sh'..."
bash "$SCRIPT_DIR/all-in-one_update.sh"

echo "chamando o script 'all-in-one_rings.sh'..."
bash "$SCRIPT_DIR/all-in-one_rings.sh"

FIM=date
echo "Processo iniciado em $INICIO e concluído em $FIM"
