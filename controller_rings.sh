...
certificado="$SCRIPT_DIR/ubuntu24.pem"
...
echo "tentando tranferir dados por SSH com senha para o compute1..."
...
echo "tentando tranferir dados por SSH com certificado para o compute1..."
sudo scp -i $certificado /etc/swift/swift.conf /etc/swift/account.ring.gz /etc/swift/container.ring.gz /etc/swift/object.ring.gz $USUARIO@"$ip_object1":/tmp/
ssh -i $certificado $USUARIO@"$ip_object1" "sudo bash /home/$USUARIO/abc/object_rings.sh"
