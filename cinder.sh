...
sudo apt install cinder-volume cinder-backup -y &>/dev/null
...
[backup]
backup_swift_auth_url = http://${controller[0]}:5000/v3
backup_swift_auth_version = 3
backup_swift_user = swift
backup_swift_key = $senha
backup_swift_tenant = service
backup_driver = cinder.backup.drivers.swift.SwiftBackupDriver
backup_swift_url = http://${controller[0]}:8080/v1/
EOF"
...
if [[ "$host_temp" = "controller" && "$all_in_one" = "sim" ]]; then
echo "configuração do banco de dados para o cinder"
sudo mysql <<EOF
CREATE DATABASE cinder;
GRANT ALL PRIVILEGES ON cinder.* TO 'cinder'@'localhost' IDENTIFIED BY '$senha';
...
echo "instalando Cinder"
# Instalar e configurar Cinder
sudo apt install cinder-api cinder-scheduler cinder-volume cinder-backup -y &>/dev/null
sudo apt install tgt -y &>/dev/null
echo "configurando o arquivo /etc/cinder/cinder.conf... "
...
echo "reiniciando serviços do Cinder e apache/keystone"
sudo systemctl restart cinder-scheduler cinder-backup cinder-volume
sudo service apache2 restart
